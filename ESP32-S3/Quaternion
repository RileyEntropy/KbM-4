...
// by Riley Amber :)
// Updated by Vance with quaternion rotation and Euler angle debugging
...
void quaternionToEuler(const struct Quaternion &q, float &roll, float &pitch, float &yaw);
...
//     Gyro Custom Defines
//=============================
// Use quaternion rotation to remap gyro axes.
// Define the desired rotation angle (in degrees) about the Z-axis.
// For no rotation, set to 0; for a 90° counterclockwise, set to 90;
// for 90° clockwise, set to -90; for a 180° flip, set to 180.
#define QUATERNION_ROTATION_DEG 90
...
Quaternion quaternionFromAxisAngle(float angleDeg, float axisX, float axisY, float axisZ) {
  float angleRad = angleDeg * (M_PI / 180.0);
  float halfAngle = angleRad / 2.0;
...
void setup() {
  ...
  delay(2000); // Ensure serial connection is established
  ...
  for (int i = 0; i < sizeof(gpioPins) / sizeof(gpioPins[0]); i++) {
    ...
    previousStates[i] = HIGH; // Not pressed
  }
...
void handleIMUMovement() {
  ...
  // Rotate the gyro vector using quaternion rotation (about Z-axis)
  if (QUATERNION_ROTATION_DEG != 0) {
    Quaternion q = quaternionFromAxisAngle(QUATERNION_ROTATION_DEG, 0, 0, 1);
    Quaternion qConj = conjugateQuaternion(q);
    Quaternion p = {0, deltaX, deltaY, deltaZ};
    Quaternion rotated = multiplyQuaternion(multiplyQuaternion(q, p), qConj);
    deltaX = rotated.x;
    deltaY = rotated.y;
    deltaZ = rotated.z;
    
    // Debug: Convert the rotation quaternion to Euler angles for verification
    float roll, pitch, yaw;
    quaternionToEuler(q, roll, pitch, yaw);
    Serial.print("Rotation Euler angles (deg): roll=");
    Serial.print(roll * 180.0 / M_PI);
    Serial.print(" pitch=");
    Serial.print(pitch * 180.0 / M_PI);
    Serial.print(" yaw=");
    Serial.println(yaw * 180.0 / M_PI);
  }
